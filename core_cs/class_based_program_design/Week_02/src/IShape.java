 ...
 * Exercise
 * 
 * Define a new shape, Combo, that contains two IShapes. Its distanceToOrigin
 * should be the minimum of the distances of its two components, its area should
 * be the sum of the areas of its two components, it can grow by growing both of
 * its components, and it contains a CartPt if either of its components contains
 * the given point.
 */
...
class Combo implements IShape {
	...
	public double distToOrigin() {
		...
		double shape1Dist = this.shape1.distToOrigin();
		double shape2Dist = this.shape2.distToOrigin();

		return shape1Dist > shape2Dist ? shape2Dist : shape1Dist;
	}
	...
	@Override
	public boolean contains(CartPt pt) {
		return this.shape1.contains(pt) || this.shape2.contains(pt);
	}
