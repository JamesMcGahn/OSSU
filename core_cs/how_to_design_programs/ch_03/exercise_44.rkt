...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname exercise_44) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
; WS Number Number String-> WS
; when mouse button clicked moves car to x location of the mouse
; otherwise returns WS
(check-expect (click 0 100 100 "button-down") 100)
(check-expect (click 700 100 100 "button-up") 700)
(check-expect (click 0 700 100 "button-down") 700)

(define (click ws x y clk)
   (if (string=? clk "button-down") x ws))
