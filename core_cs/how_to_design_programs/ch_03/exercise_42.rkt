...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname exercise_42) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
; WS -> CAR
; places CAR on the background scene according to cars right most x coordindate plus WS state


(check-expect (render 0) (place-image CAR (image-width CAR) (- HEIGHT (/ (image-height CAR)2)) SCENE)
  )
(check-expect (render WIDTH-OF-WORLD) (place-image CAR (+ WIDTH-OF-WORLD  (image-width CAR))(- HEIGHT (/ (image-height CAR)2)) SCENE)
  )
(check-expect (render (/ WIDTH-OF-WORLD 2)) (place-image CAR (+ (/ WIDTH-OF-WORLD 2) (image-width CAR)) (- HEIGHT (/ (image-height CAR)2)) SCENE)
  )

(define (render ws)

(place-image CAR (+ ws (image-width CAR)) (- HEIGHT (/ (image-height CAR)2)) SCENE)
  )
...
(define (stop ws)
  (>= (+ ws  (* (image-width CAR) 1.5)) WIDTH-OF-WORLD))
