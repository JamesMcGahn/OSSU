...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname exercise_177) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (render editor)
  ...
  (overlay/align "left" "center"
                 (beside 
                  (text (if (empty? (editor-pre editor)) "" (implode (rev (editor-pre editor)))) 16 "black")
                  (rectangle 1 20 "solid" "red")
                  (text (if (empty? (editor-post editor)) "" (implode (editor-post editor))) 16 "black") 
                  )
                 (empty-scene 200 20))
  )
...
(define (edit ed ke )
  (cond [(and (> (string-length (implode (editor-pre ed))) 0) (string=? "\b" ke)) (make-editor (editor-delete (editor-pre ed))  (editor-post ed)]
        [(and (> (string-length (implode (editor-pre ed))) 0) (string=? "left" ke))  (make-editor (substring (editor-pre ed) 0 (- (string-length (editor-pre ed)) 1)) (string-append (substring (editor-pre ed) (- (string-length (editor-pre ed)) 1) ) (editor-post ed) ))]
        [(and (> (string-length (implode (editor-post ed))) 0) (string=? "right" ke)) (make-editor (string-append (editor-pre ed) (substring (editor-post ed) 0 1)) (substring (editor-post ed) 1))]
        [(and
