...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname exercise_114) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(check-expect (isZoo? (make-zoo (make-VCat 0 1 100) (make-Vcham 0 100 "black") "cham")) true)
(check-expect (isZoo? (make-zoo (make-VCat 0 1 100) (make-Vcham 0 100 "black") "cat")) true)
(check-expect (isZoo? (make-zoo (make-VCat 0 1 100) (make-Vcham 0 100 "black") "")) true)
(check-error (isZoo? (make-zoo 34 (make-Vcham 0 100 "black") "")) "needs to be of type zoo")
(check-error (isZoo? (make-zoo (make-VCat 0 1 100) "not cham" "")) "needs to be of type zoo")
(check-error (isZoo? (make-zoo (make-VCat 0 1 100) (make-Vcham 0 100 "black") "ads")) "needs to be of type zoo")

(define (isZoo? s)
  (cond [(and (Vcham? (zoo-cham s)) (VCat? (zoo-cat s)) (or (string=? (zoo-animal s) "cham" ) (string=? (zoo-animal s) "cat" ) (string=? (zoo-animal s) "" ))) true]
  [else (error "needs to be of type zoo")])
)
