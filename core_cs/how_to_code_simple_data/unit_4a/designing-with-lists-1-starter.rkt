...
;; ListOfNumber -> Number
;; produce sum of total owls weights
(check-expect (count empty) 0)
(check-expect (count (cons 20 empty)) 1)
(check-expect (count (cons 32 (cons 20 empty))) 2)
(check-expect (count (cons 31 (cons 32 (cons 20 empty)))) 3)
